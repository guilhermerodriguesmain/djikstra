...
//-------------------------------------------------------------Atualiza digitando matriz (Textarea)
...
async function calculateRoute() {
    ...
    try {
        ...
        // A resposta contém: { result: {...}, graph: {...} }
...
//---------------------------------------------------- Recebe os dados do Python e atualiza a tela
function render(data) {
    // 1. Atualiza a Imagem (O Python precisa enviar a string Base64)
    if (data.image) {
...
async function saveFromTable(size) {
    ...
    try {
        ...
        const data = await res.json();
        if(data.status === 'ok') {
            // Atualiza a imagem e o textarea, mas mantém a tabela.
            if (data.graph.image) {
